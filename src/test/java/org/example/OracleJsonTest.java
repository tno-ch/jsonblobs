...
import org.example.oracle.EntityValueJson;
...
public class OracleJsonTest {
    ...
    void testAllJson() throws IOException {
        ...
        //List<EntityValueJson> result = em.createNativeQuery( "select * from bro_dba.test_entity_value", org.example.oracle.EntityValueJson.class ).getResultList();
        //System.out.println( result );
        em.createNativeQuery( "delete from bro_dba.test_entity_value" ).executeUpdate();
        em.flush();
        log.info( "Truncate table: bro_dba.test_entity_value" );
    ...
    private void read(int max, Type type) {
        ...
        for ( int i = 0; i < max; i++ ) {
            ...
            assertThat( entityValueJson.getPayload() ).isNotNull();
        }
        long total = Duration.between( start, Instant.now() ).toMillis();
        BigDecimal avg = new BigDecimal( total ).divide( new BigDecimal( max ), 3, RoundingMode.HALF_UP );
