...
import org.example.bytea_lz4.EntityValueByteaLz4;
import org.junit.jupiter.api.Test;

@QuarkusTest
@Slf4j
public class PostgresByteaLz4Test {
    ...
    void testAllJson() throws IOException {
        ...
        em.createQuery( "delete from EntityValueByteaLz4" ).executeUpdate();
        em.flush();
        log.info( "Truncate table: EntityValueByteaLz4" );
    ...
    private void insert(String json, int max, Type type) {
        ...
        Instant start = Instant.now();
        for ( int i = 0; i < max; i++ ) {
            EntityValueByteaLz4 byteaLz4 = EntityValueByteaLz4.builder()
                                                              .id( type.name() + "-" + i )
    ...
    private void read(int max, Type type) {
        Instant start = Instant.now();
        for ( int i = 0; i < max; i++ ) {
            EntityValueByteaLz4 byteaLz4 = em.find( EntityValueByteaLz4.class, type.name() + "-" + i );
            assertThat( byteaLz4 ).isNotNull();
            assertThat( byteaLz4.getPayload() ).isNotNull();
        }
        long total = Duration.between( start, Instant.now() ).toMillis();
        BigDecimal avg = new BigDecimal( total ).divide( new BigDecimal( max ), 3, RoundingMode.HALF_UP );
